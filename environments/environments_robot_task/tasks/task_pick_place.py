...
class TaskPickPlace(Task):
    def __init__(
        ...
        **kwargs
        ...
        self.limits = np.array([(0.2, 0.8), (-0.8, 0.8), (0.0, 0.8)])

        self.state_space = spaces.Dict(
            {
                # "position": spaces.Box(-1, 1, shape=(3,)),
                "object_gripped": spaces.Box(-1, 1, shape=(1,), dtype=np.float64),
            }
        )
    ...
    def reset(
        ...
        while True:
            ...
            desired_state_object[-1] = 0.01  # put on floor
    ...
    def step(self, state_robot=None, robot=None):
        ...
        if state_robot is not None and robot is not None:
            if state_robot["hand"] == robot.status_hand.OPEN.value:  # open
                ...
                if np.linalg.norm(tcp_position - np.array(position_object)) < 0.03:
                    self.tcp_object_constraint = self.bullet_client.createConstraint(
                        self.object,
                        -1,
                        robot.model_id,
