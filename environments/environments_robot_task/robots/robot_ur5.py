...
class RobotUR5(Robot):
    ...
    def inverse_kinematics(self, T_0E):
        ...
        # Theta 1
        theta_1_a = torch.atan2(P_5[:, 1:2], P_5[:, 0:1]) + .5 * torch.pi
        theta_1_b = torch.acos(self.dht_params[3]["d"] / (P_5[:, 0:1] ** 2 + P_5[:, 1:2] ** 2) ** .5)
        ...
        T_05 = torch.einsum("axy,apyz->apxz", T_06,
                            self.dht_model.transformations[5](theta_6.reshape(-1, 1)).reshape(*theta_6.shape, 4,
                                                                                              4).inverse())
        T_04 = torch.einsum("apxy,apyz->apxz", T_05,
                            self.dht_model.transformations[4](theta_5.reshape(-1, 1)).reshape(*theta_5.shape, 4,
                                                                                              4).inverse())
        ...
        theta_3_ = torch.acos((P_13_xy_length ** 2 - self.dht_params[1]["a"] ** 2 - self.dht_params[2]["a"] ** 2) /
        ...
        solutions = torch.stack((
            theta_1.repeat(1, 4), theta_2, theta_3, theta_4, theta_5.repeat(1, 2), theta_6.repeat(1, 2)
        ), dim=-1)
