...
class Expert:
    def __init__(self, env):
        ...
        assert len(self.ik_dof_joint_ids) == len(
            self.robot.joints), "Mismatch between specified DOF and DOF found by Klampt!"
    ...
    def predict(self, state, goal):
        ...
        if goal_position is not None:
            ...
            if goal_orientation is None:
                obj = ik.objective(self.ik_model.link(self.ik_model.numLinks() - 1), local=[0, 0, 0],
                                   world=list(goal_position))
            else:
                obj = ik.objective(self.ik_model.link(self.ik_model.numLinks() - 1),
            ...
            desired_state_arm_normed = np.array([np.interp(delta_pose, joint.limits, [-1, 1])
