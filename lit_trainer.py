...
class LitTrainer(Trainer):
    ...
    def train(self):
        with wandb.init(
            ...
            self.run_id = run.id
            callbacks = [ModelCheckpoint(monitor=f"validation_loss_{self.model_cls}{self.model_config['log_suffix']}",
                                         mode="min", filename=f"checkpoint_{self.hash}.ckpt")]
