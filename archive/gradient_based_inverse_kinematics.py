...
def perform_gradient_decent_ik(data_file, notes=None):
    ...
    with wandb.init(notes=notes, project=f"dht_ik_{os.path.basename(data_file_A).replace('.pt', '')}", entity="bitter",
                    mode=wandb_mode):
        ...
        optimizer = torch.optim.AdamW([angles], lr=1e-3)
...
if __name__ == '__main__':
    ...
    if sweep:
        ...
        train_state_mapping(config, project="robot2robot_state_mapper")
